...
//    }
//    
    func sendDriverLocation(location: CLLocation, completionHandler: () -> Void) {
    ...
    func getDriverLocation(completionHandler: (location: CLLocationCoordinate2D) -> Void) {
        socket.on("driverLocationUpdate") {(dataArray, socketAck) -> Void in
            let result = dataArray[0] as! Dictionary<String,Double>
            completionHandler(location: CLLocationCoordinate2D(latitude: result["lat"]!, longitude: result["long"]!))
        }
    }
